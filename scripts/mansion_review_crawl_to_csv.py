...
def _extract_list_row_cells(card: Node, kind: str) -> dict[str, str]:
    ...
    def _extract_chintai_recommend_row_direct(table_node: Node) -> dict[str, str]:
        tbody = table_node.css_first("tbody.recommend_row")
        if not tbody:
            return {}
        row = tbody.css_first("tr") or tbody
        cells = [normalize_space(c.text(separator=" ")) for c in row.css("td")]
        if len(cells) < 9:
            return {}
        return {
            "賃料(管理費)": cells[2],
    ...
    for table in tables:
        ...
        if kind == "chintai":
            if not has_rent:
                continue
            direct_mapped = _extract_chintai_recommend_row_direct(table)
            if direct_mapped:
                ...
                if detail_link:
                    direct_mapped["__detail_href"] = _normalize_href(detail_link.attributes.get("href", ""))
...
def parse_list_page(html: str, page_url: str, kind: str, city_id: str, page_no: int) -> tuple[list[ListRow], ParseDebug]:
    ...
    for card in cards:
        ...
        if kind == "chintai" and has_recommend_rows and not price_or_rent_text:
            warn_name = building_name or "(unknown)"
            print(f"[WARN] recommend_row exists but rent empty: building={warn_name} page={page_url}")
