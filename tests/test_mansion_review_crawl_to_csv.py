...
def test_parse_list_page_chintai_reads_recommend_row_fixed_td_positions_with_update_cell() -> None:
    html = """
    <html><body>
      <li class="property-detail-list-item">
        <h3>MDI Vista mare門司駅前</h3>
        <table class="recommendTable">
          <thead>
            <tr>
              <th colspan="3">このマンションの【賃貸】物件情報</th>
              <th>賃料(管理費)</th><th>敷金</th><th>礼金</th><th>専有面積</th><th>間取り</th><th>所在階</th><th>向き</th>
            </tr>
          </thead>
          <tbody class="recommend_row">
            <tr>
              <td><img src="/img/common/floorplan-preparing-list.png"></td>
              <td><a href="/chintai/1113660051402.html">MDI Vista mare門司駅前 303</a></td>
              <td>4.8万円<br>(4,000円)</td><td>無</td><td>2ヶ月</td><td>28.26㎡</td><td>1K</td><td>3階</td><td>北東</td>
              <td class="recommend_update_row">情報取得日 2026年3月17日</td>
            </tr>
          </tbody>
        </table>
      </li>
    ...
    assert len(rows) == 1
    row = rows[0]
    assert row.price_or_rent_text == "4.8万円"
    assert row.fee_text == "4,000円"
    assert row.deposit_text == "無"
    assert row.key_money_text == "2ヶ月"
    assert row.area_text == "28.26㎡"
    assert row.layout_text == "1K"
    assert row.floor_text == "3階"
    assert row.direction_text == "北東"
