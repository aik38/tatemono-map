...
def _start_ingest_run(conn, source: str, snapshot_key: str) -> int:
    conn.execute(
        """
        INSERT INTO ingest_runs(source, snapshot_key, status, started_at)
    ...
    row = conn.execute("SELECT id FROM ingest_runs WHERE source=? AND snapshot_key=?", (source, snapshot_key)).fetchone()
    if row is None:
        raise RuntimeError("failed to create ingest run")
    return int(row[0])


def set_current_snapshot(conn, source: str, ingest_run_id: int) -> None:
    row = conn.execute("SELECT status FROM ingest_runs WHERE id=?", (ingest_run_id,)).fetchone()
    if row is None or row[0] != "completed":
        raise RuntimeError(f"run is not completed: {ingest_run_id}")
    conn.execute(
        """
        INSERT INTO current_ingest_snapshots(source, ingest_run_id, switched_at)
...
def ingest_master_import_csv(db_path: str, csv_path: str, source: str = "master_import") -> Report:
    ...
    report.ingest_run_id = ingest_run_id
    ...
    try:
        ...
        conn.rollback()
        conn.execute("UPDATE ingest_runs SET status='failed', finished_at=CURRENT_TIMESTAMP WHERE id=?", (ingest_run_id,))
        conn.commit()
        conn.close()
        raise
...
def main() -> None:
    ...
    if args.set_current_run_id:
        ...
        try:
            set_current_snapshot(conn, args.source, int(args.set_current_run_id))
            conn.commit()
        finally:
            conn.close()
